...
async def main(packet):
    ...
    if init_nn_s == 2:                                  #人脸
        ...
        yolo2_decoder = decoder.Yolo2(len(labelss), anchors, net_in_size=(224, 224), net_out_size=(7, 7))
        init_nn_s = 0
    ...
    if packet["selected"] == "demo3":                       #分类
        ...
        boxes, probs = yolo2_decoder.run(out, nms=0.3, threshold=0.5, img_size=(224, 224))
        if len(boxes):
            t = time.time()
